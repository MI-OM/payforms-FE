...
export function ImportContacts() {
  ...
  const handleImport = async () => {
    ...
    try {
      const contacts = parsedData.map(mapRowToImportContact).filter(c => c.email)
      const { import_id } = await contactService.importContacts(contacts)
      setImportId(import_id)
      
      const result = await contactService.commitImport(import_id)
            ...
            <>
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  <strong>CSV Format:</strong> Include columns for name and email. Optional: phone.
                </p>
              </div>
            </>
          )}
